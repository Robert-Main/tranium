...
export default function NotesSection({ companionId, sessionId, path, initialNotes }: NotesSectionProps) {
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-sm font-semibold text-slate-900">Add a note</FormLabel>
                                    <FormControl>
                                        <Textarea
                                            placeholder="Capture your thoughts, key takeaways, or questions..."
                                            className="min-h-[100px] resize-none bg-slate-50 border-slate-200 focus:border-slate-400 focus:ring-2 focus:ring-slate-200 rounded-xl text-sm"
                                            {...field}
                                        />
                                    </FormControl>
                                    <FormMessage />
                        ...
                        <div className="flex justify-end">
                            <Button
                                type="submit"
                                disabled={isPending}
                                className="bg-slate-900 hover:bg-slate-800 text-white shadow-sm hover:shadow-md transition-all duration-200 rounded-xl"
                            >
                                {isPending ? (
                                    <>
            ...
            {canMultiSelect && (
                <div className="flex items-center justify-between gap-2 px-1">
                    ...
                    {multiSelect && selectedIds.size > 0 && (
                        <ConfirmModal
                            trigger={
                                <Button variant="destructive" size="sm" disabled={bulkDeleting} className="rounded-xl">
                                    {bulkDeleting ? (
                                        <>
                                            <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                                            Deleting...
            ...
            <div className="space-y-3">
                {initialNotes?.length === 0 ? (
                    <div className="text-center py-16 px-4 bg-slate-50 border border-dashed border-slate-300 rounded-2xl">
                        ...
                        <p className="text-sm font-medium text-slate-900 mb-1">No notes yet</p>
                        <p className="text-xs text-slate-500">Start capturing your thoughts above</p>
                    </div>
                ) : (
                    <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1 custom-scrollbar overscroll-contain">
                        {initialNotes?.map((note) => (
                            <div
                                key={note.id}
                                className={`group relative bg-white border rounded-xl p-4 transition-all duration-200 ${
                                    multiSelect && selectedIds.has(note.id)
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        {editingId === note.id ? (
                                            ...
                                            <>
                                                <p className="whitespace-pre-wrap text-sm text-slate-700 leading-relaxed mb-2">
                                                    {note.content}
                                                </p>
